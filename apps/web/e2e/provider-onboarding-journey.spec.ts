...
test.describe('Provider Onboarding Journey', () => {
  test('Complete provider onboarding: application → vetting → profile setup → program creation', async ({ page }) => {
    ...
    await test.step('Submit provider application', async () => {
      ...
      await expect(page.locator('[data-testid="success-message"]')).toContainText(/application.*received/i);
    });
    ...
    await test.step('Provider completes profile', async () => {
      ...
      await page.fill('[data-testid="special-needs-details"]', 'Wheelchair accessible, trained staff');
