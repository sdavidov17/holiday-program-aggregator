...
export default function ProvidersListPage() {
  ...
  const { data: providers, refetch, isLoading } = api.provider.getAll.useQuery({
    includeUnpublished,
    includeUnvetted,
  });
      ...
      <div className="overflow-hidden rounded-lg bg-white shadow">
              ...
              <tr key={provider.id}>
                ...
                <td className="px-6 py-4 text-sm text-gray-500">
                  {provider.suburb && provider.state ? (
                    <div>{provider.suburb}, {provider.state}</div>
                  ) : (
                ...
                <td className="px-6 py-4">
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      disabled={(!provider.isVetted && !provider.isPublished) || toggleVetting.isPending || togglePublishing.isPending}
                      className={`inline-flex rounded-full px-2 text-xs font-semibold leading-5 ${
        ...
        {(!providers || providers.length === 0) && (
          <div className="px-6 py-8 text-center text-gray-500">
            No providers found. <Link href="/admin/providers/new" className="text-blue-600 hover:text-blue-800">Add your first provider</Link>
          </div>
        )}
      </div>
      )}
